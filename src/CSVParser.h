...
private:
    ...
    void addRecordValue(bool lastRecord) {
        ...
        string inputClean = _format->getTrim() ? utils::trimTrailingSpaces(input) : input;
        if (lastRecord && inputClean.empty() && _format->getTrailingDelimiter()) {
            return;
        }
        string nullString = _format->getNullString();
...
//                    new LinkedHashMap<String, Integer>();
            ...
            if (!headerRecord.empty()) {
                for (unsigned i = 0; i < headerRecord.size(); i++) {
                    ...
                    bool emptyHeader = utils::trimTrailingSpaces(header).empty();
